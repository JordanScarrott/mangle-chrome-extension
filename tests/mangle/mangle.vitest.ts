...
const MANGLE_WASM_PATH = path.resolve(__dirname, '../../src/mangle/mangle.wasm');
...
describe('Mangle WASM Module', () => {
  ...
  it('should define and query simple facts', () => {
  ...
  it('should return an error for invalid queries', () => {
    const errResult = mangleQuery('foo(');
    expect(errResult.startsWith('Error:')).toBe(true);
  });

  it('should handle more complex queries and rules', () => {
    ...
    mangleDefine(rule);
    const result3 = mangleQuery('visitorAndLocal("Mia", Name, Destination)');
