...
async function extractMainContent() {
  // First, we need to dynamically import our Readability library.
  // We use chrome.runtime.getURL to get the correct, extension-specific path.
  const readabilityUrl = chrome.runtime.getURL("src/lib/Readability.js");
  const { Readability } = await import(readabilityUrl);
